...
# Read custom text chunk from PNG with error handling
...
def decode_with_authentication(imgs, imgs_extra, index, r, expected_watermark):
    ...
    for i in range(r):
        ...
        try:
            watermark = read_text_chunk(secret_img_path) 
...
# Compare two images and plot the comparison
def compare_images(image1_path, image2_path):
    ...
    pixel_indices = np.arange(len(flattened_diff))

    # Plot the difference
    plt.figure(figsize=(12, 6))
    plt.plot(pixel_indices, flattened_diff, label="Pixel Differences", color="blue", alpha=0.7)
    plt.axhline(y=np.mean(flattened_diff), color="red", linestyle="--", label=f"Mean Difference: {diff_value}")
    plt.title("Pixel-wise Difference Between Images")
...
def main():
    ...
    if args.encode:
        ...
        print(f"=== Image encoding completed. Time elapsed: {elapsed_time:.2f} seconds ===")
    ...
    if args.decode:
        ...
        end_time = time.time()
        elapsed_time = end_time - start_time
        print(f"=== Image decoding completed. Time elapsed: {elapsed_time:.2f} seconds ===")
